...
if [[ "${export_priv:-N}" =~ ^[Yy]$ ]]; then
  ...
  echo "  3. Paste: $passphrase"
  echo ""
  echo "In your GitHub Actions workflow, use:"
  echo "  - name: Import GPG key"
  echo "    run: |"
  echo "      echo \"\${{ secrets.GPG_SIGNING_KEY }}\" | gpg --import"
  echo ""
  echo "  - name: Sign and publish"
  echo "    run: |"
  echo "      export GPG_TTY=\$(tty)"
  echo "      export GPG_SIGNING_PASSWORD=\${{ secrets.GPG_SIGNING_PASSWORD }}"
  echo "      # your publish command here"
  echo "    env:"
  echo "      GPG_SIGNING_PASSWORD: \${{ secrets.GPG_SIGNING_PASSWORD }}"
  echo ""
fi
